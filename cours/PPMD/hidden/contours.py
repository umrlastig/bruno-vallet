...
def ContoursCanny(image, sigma, seuil_bas, seuil_haut):
    ...
    sm.imsave(img_filename+'.gc.jpg', grad_c_img)
    grad_l_img = sp.ndimage.convolve(gauss_img, grad_l)
    sm.imsave(img_filename+'.gl.jpg', grad_l_img)
    grad_norm = np.sqrt(
       np.multiply(grad_c_img, grad_c_img)+
       np.multiply(grad_l_img, grad_l_img))
    sm.imsave(img_filename+'.gnorm.jpg', grad_norm)
    grad_s = 0.382683432*grad_norm
    gs_c = snap(grad_c_img / grad_s)
    #sm.imsave(img_filename+'.gsc.jpg', gs_c)
    gs_l = snap(grad_l_img / grad_s)
    #sm.imsave(img_filename+'.gsl.jpg', gs_l)
...
def ContoursLaplace(image, sigma, seuil_bas, seuil_haut):
    ...
    sm.imsave(img_filename+'.L.jpg', L_img)
    contours = np.zeros(image.shape, dtype=np.bool)
...
image = sm.imread(img_filename)
print(image.shape)
canny = ContoursCanny(image, sigma, seuil_bas, seuil_haut)
print('Writing %s' % canny_filename)
sm.imsave(canny_filename, canny)
laplace = ContoursLaplace(image, sigma, seuil_bas, seuil_haut)
print('Writing %s' % lapl_filename)
sm.imsave(lapl_filename, laplace)
